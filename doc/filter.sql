...
DELETE FROM causalehr.causalpredication 
WHERE subject_cui IN (SELECT cui FROM generic_cuis)
   OR object_cui IN (SELECT cui FROM generic_cuis);



WITH excluded_semtypes AS (
    SELECT unnest(ARRAY[
...
DELETE FROM causalehr.causalpredication 
WHERE subject_semtype IN (SELECT semtype FROM excluded_semtypes)
   OR object_semtype IN (SELECT semtype FROM excluded_semtypes);