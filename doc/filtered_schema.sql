...
-- Drop and recreate the filtered predication table
DROP TABLE IF EXISTS filtered.predication;
CREATE TABLE filtered.predication AS
WITH generic_cuis AS (
    SELECT unnest(ARRAY[
...
excluded_semtypes AS (
    SELECT unnest(ARRAY[
        'acty', 'bhvr', 'evnt', 'gora', 'mcha', 'ocac', 'clas', 'cnce', 'ftcn', 'grpa',
        'idcn', 'inpr', 'lang', 'qlco', 'qnco', 'rnlw', 'spco', 'tmco', 'enty', 'mnob',
        'phob', 'bmod', 'ocdi', 'hcro', 'orgt', 'pros', 'shro', 'eehu', 'hcpp', 'npop', 'phpr'
    ]) AS semtype
)
SELECT *
FROM predication cp
WHERE cp.subject_cui NOT IN (SELECT cui FROM generic_cuis)
  AND cp.object_cui NOT IN (SELECT cui FROM generic_cuis)
  AND cp.subject_semtype NOT IN (SELECT semtype FROM excluded_semtypes)
  AND cp.object_semtype NOT IN (SELECT semtype FROM excluded_semtypes);
